...
async def generate_job(
    ...
    job = await user_service.create_job(job_name, description)
    return job
...
async def generate_level(
    ...
    """레벨 생성 헬퍼 함수"""
    user_service = UserService(db)
    level = await user_service.create_level(level_name, description)
    return level
...
async def change_password(
    ...
    # 현재 비밀번호 확인
    if not await user_service.verify_user_password(current_user.user_id, password_change.current_password):
        raise HTTPException(
    ...
    # 새 비밀번호로 변경
    updated_user_password = await user_service.update_user_password(current_user.user_id, password_change.new_password)
...
async def change_language(
    ...
    """모국어 변경"""
    user_service = UserService(db)
    updated_user_language = await user_service.update_user_language(current_user.user_id, language_change.nationality)
...
async def change_job(
    ...
    """직무 변경"""
    user_service = UserService(db)
    updated_user_job = await user_service.update_user_job(current_user.user_id, job_change.job_id)
...
async def get_user_status(
    ...
    """사용자 학습 상태(통계) 조회"""
    user_service = UserService(db)
    user_status = await user_service.get_user_status(user_id)
